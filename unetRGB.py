...
class myUnet(object):
    ...
    def weightedLoss(self, originalLossFunc, weightsList):
        def lossFunc(true, pred):
            ...
            classSelectors = [K.equal(i, classSelectors)
    ...
    def loss_angle(self, y_true, y_pred):
        ...
        for i, c in enumerate(clusters):  # Select one candidate per cluster
            ...
            # choose min if angle<90 or max if angle>90
            slines1.append(
                (k*max(list(clusters[i]))[0])+((1-k)*min(list(clusters[i]))[0]))
    ...
    def get_unet(self):
        ...
        weights = [1, 2]
        model.compile(optimizer=Adam(lr=1e-4), loss=self.weightedLoss(tf.keras.losses.BinaryCrossentropy(
            from_logits=True), weights), metrics=['accuracy', self.iou, self.loss_angle], run_eagerly=True)
    ...
    def save_img(self):
        ...
        for i in range(imgs.shape[0]):
            ...
            cv_pic = cv2.resize(cv_pic, (512, 512),
                                interpolation=cv2.INTER_CUBIC)
            binary, cv_save = cv2.threshold(
                cv_pic, 127, 255, cv2.THRESH_BINARY)
