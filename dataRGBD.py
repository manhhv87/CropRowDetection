...
class dataProcess(object):
    ...
    def create_train_data(self):
        ...
        imgdatas = np.ndarray(
            (len(imgs), self.out_rows, self.out_cols, 4), dtype=np.uint8)
        imglabels = np.ndarray(
            (len(imgs), self.out_rows, self.out_cols, 1), dtype=np.uint8)
        ...
        for x in range(len(imgs)):
            ...
            depth = load_img(depthpath, color_mode='grayscale',
                             target_size=[512, 512])
            label = load_img(labelpath, color_mode='grayscale',
                             target_size=[512, 512])
    ...
    def create_test_data(self):
        ...
        imgs = glob.glob(self.test_path + "/*." + self.img_type)
        imgdatas = np.ndarray(
            (len(imgs), self.out_rows, self.out_cols, 3), dtype=np.uint8)
        testpathlist = []
